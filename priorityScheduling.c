...
Process *findProcessWithHighestPriorityWithinWindow();
...
void takeInput() {
	...
	for (int i = 0; i < processesCount; i++) {
		...
		scanf("%d", &currentProcess->priority);
	}
}
...
void scheduleProcesses() {
	...
	while (unExecutedProcessesWithInCurrentTimeWindowSize() > 0) {
		Process *nextProcessToBeExecuted = findProcessWithHighestPriorityWithinWindow();
...
Process *findProcessWithHighestPriorityWithinWindow() {
	...
	for (int i = windowStart + 1; i <= windowEnd; i++) {
		if (processes[i].priority < resultProcess->priority)
